...
export default function TrainingPlan() {
  ...
  const [checked, setChecked] = useState(() => loadLocal("marathon-checked"));
  const [adjust, setAdjust] = useState(() => loadLocal("marathon-adjust"));
  ...
  useEffect(() => {
    fetch("/api/progress")
      ...
      .then((data) => {
        ...
        if (data.adjust) setAdjust(data.adjust);
  ...
  // Debounced save to server whenever state changes (after initial load)
  useEffect(() => {
    if (!loadedRef.current) return;
    saveLocal("marathon-checked", checked);
    saveLocal("marathon-adjust", adjust);
    ...
    saveTimerRef.current = setTimeout(() => {
      fetch("/api/progress", {
        ...
        body: JSON.stringify({ checked, adjust }),
      })
        .then((r) => (r.ok ? setSyncStatus("synced") : setSyncStatus("offline")))
  ...
  const toggleRun = (weekIdx, runIdx) => {
    const key = `${view}-${weekIdx}-${runIdx}`;
    setChecked((prev) => ({ ...prev, [key]: !prev[key] }));
  };

  const adjustKey = view;
  const currentAdjust = adjust[adjustKey] || 0;
  const changeAdjust = (delta) => {
    setAdjust((prev) => ({ ...prev, [adjustKey]: (prev[adjustKey] || 0) + delta }));
  };

  // Calculate total km from checked runs
  const totalRan = WEEKS.reduce((sum, w, wi) => {
    const weekRuns = view === "her" ? w.her : w.him;
    return sum + weekRuns.reduce((s, run, ri) => {
      const key = `${view}-${wi}-${ri}`;
      return s + (checked[key] && typeof run.km === "number" ? run.km : 0);
    }, 0);
  }, 0) + currentAdjust;
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        ...
        <div style={{ fontSize: 9, color: "#bbb", marginTop: 6, letterSpacing: 1 }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {runs.map((run, i) => {
            ...
            const runKey = `${view}-${selectedWeek}-${i}`;
            const done = !!checked[runKey];
            return (
              <div
                key={i}
                style={{
              ...
              >
                <div
                  onClick={() => toggleRun(selectedWeek, i)}
                  style={{
                    width: 48,
                    display: "flex",
                    ...
                    background: done ? "#e0e0e0" : colors.bg,
                    padding: "10px 0",
                    borderRight: `1px solid ${colors.border}`,
                    cursor: "pointer",
                  }}
                >
                  <div style={{ fontSize: 12, fontWeight: 700, color: done ? "#888" : colors.text }}>
                    {done ? "\u2713" : run.day}
                  </div>
                </div>
                <div style={{ flex: 1, padding: "10px 14px", background: done ? "#fafafa" : "#fff" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          color: "#999",
          marginBottom: 12,
        }}>Total Ran ({view === "her" ? "Her" : "Him"})</div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16 }}>
          <button
            onClick={() => changeAdjust(-1)}
            style={{
          ...
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 28, fontWeight: 700, fontFamily: "'Courier New', monospace" }}>
              {totalRan.toFixed(1)}
            </div>
            <div style={{ fontSize: 9, color: "#999", letterSpacing: 1, textTransform: "uppercase" }}>km total</div>
            {currentAdjust !== 0 && (
              <div style={{ fontSize: 10, color: "#aaa", marginTop: 2 }}>
                ({currentAdjust > 0 ? "+" : ""}{currentAdjust}km adjusted)
              </div>
            )}
          </div>
          <button
